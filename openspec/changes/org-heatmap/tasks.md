...
- [ ] 5.2 Import `Countries.csv` and `Subdivisions.csv` as new tabs in the live sheet; verify the stamp row is present.
- [ ] 5.3 Rename or add columns on the data tab so the header row matches the schema: `Organization`, `Country`, `State / Province`, `Country Code`, `Subdivision Code`, `Region Code`, `Website`, `Notes`; rename the tab to `Data`.
- [ ] 5.4 Add the `Country Code`, `Subdivision Code`, and `Region Code` array formulas; confirm a new row at the bottom fills itself.
- [ ] 5.5 Add the Country dropdown validation and the two red-cell rules; walk the existing rows and clean any red cells (retype country from the dropdown, correct or blank unmatched provinces).
